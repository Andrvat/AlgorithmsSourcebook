...
Вместо того, чтобы использовать более умный пропуск (Бойер-Мур), алгоритм Рабина—Карпа пытается **ускорить проверку эквивалентности образца с подстроками в тексте, используя хеш-функцию.** *Хеш-функция* — это функция, преобразующая каждую строку в числовое значение, называемое _хеш-значением_ (или просто хеш).
