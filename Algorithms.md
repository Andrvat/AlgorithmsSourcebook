...
- Структуры данных
    ...
    - [Стек. Преобразование инфиксной записи в постфиксную. Вычисление значения выражения.](#Стек-в-применении-и-вычисление-значения-выражения)
- Алгоритмы поиска подстроки в строке
    - [Поиск подстроки в строке. Алгоритм Рабина-Карпа. Оценка числа операций.](#Алгоритм-Рабина-и-Карпа-поиска-подстроки-в-строке)
    - [Поиск подстроки	в строке. Алгоритм Бойера-Мура без суффиксной эвристики. Оценка числа операций.](#Алгоритм-Бойера-и-Мура-поиска-подстроки-в-строке)
...
## Стек в применении и вычисление значения выражения

Скажем вначале пару слов о стеке.

**Стек** — это структура данных, элементы которой получают по принципу «последний вошел, первый вышел» (Last-In-First-Out или LIFO). Это значит, что мы будем иметь доступ только к последнему добавленному элементу.

![https://s3.us-west-2.amazonaws.com/secure.notion-static.com/96eae8c7-3b10-4ec6-8759-884944f2b6ea/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20200811%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20200811T165835Z&X-Amz-Expires=86400&X-Amz-Signature=44841fd1926b2b9ff68cbb182c0d8f5cc3ca0cf40822264d6f79073b4867ab58&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22](https://s3.us-west-2.amazonaws.com/secure.notion-static.com/96eae8c7-3b10-4ec6-8759-884944f2b6ea/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20200811%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20200811T165835Z&X-Amz-Expires=86400&X-Amz-Signature=44841fd1926b2b9ff68cbb182c0d8f5cc3ca0cf40822264d6f79073b4867ab58&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22)

В отличие от списков, мы не можем получить доступ к произвольному элементу стека. Мы можем только добавлять или удалять элементы, причем если мы хотим достать элементы, например, из середины, то нам нужно будет сначала всё удалить сверху него.

Базовые операции, необходимые для работы со стеком:

![https://s3.us-west-2.amazonaws.com/secure.notion-static.com/208eb070-32be-4405-b3a3-2291e905186a/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20200811%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20200811T170024Z&X-Amz-Expires=86400&X-Amz-Signature=3cf0a102828922332d22db20b152e58e5b02c16cb1de077278f2375513a57667&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22](https://s3.us-west-2.amazonaws.com/secure.notion-static.com/208eb070-32be-4405-b3a3-2291e905186a/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20200811%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20200811T170024Z&X-Amz-Expires=86400&X-Amz-Signature=3cf0a102828922332d22db20b152e58e5b02c16cb1de077278f2375513a57667&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22)

### Основная часть вопроса.

**Инфиксной записью** мы называем привычную нам запись арифметического выражения, **где знаки операции стоят между операндами** (числами) и т.д.

Пример инфиксной записи:

_a + (f – b * c / (z – x) + y) / (a * r – k)_

**Постфиксная же (или обратная польская) запись** - это форма записи математических выражений, в которой **операнды расположены перед знаками операций**. Заметим сразу, что такая запись не нуждается в скобках для выставления приоритетов операций.

Наш пример в постфиксной записи:

_a f b c * z x – / – y + a r * k – / +_

Зачем нужна она?  **С помощью обратной польской (постфиксной записи) происходят вычисления арифметических выражений компьютером.**

Пусть на вход программе подается инфиксная, привычная нам форма записи арифметического выражения и мы хотим получить её результат. Для реализации такого, можно сказать, калькулятора нам и понадобится структура данных стек.
**ЗАМЕЧАНИЕ!** Предполагаем, что поданное на вход арифметическое выражение корректно. Иначе - атас...

Сначала научимся **переводить инфиксную запись в постфиксную:** 

1. Дадим базовым арифметическим **операциям** так называемые **приоритеты**:
...
2. Применим сам **алгоритм перевода из инфиксной записи в постфиксную**:

    ```c
    Создаем пустой стек

    Пока ещё есть символы для чтения из входной строки {
    	-- читаем очередной символ 

    	-- если символом является число (или переменная) {
    			добавляем его (её) в выходную строку
    	}
    	-- если символом является открывающаяся скобка {
    			помещаем её в стек
    	}
    	-- если символом является закрывающаяся скобка {
    				Пока верхним элементом стека не станет открывающаяся скобка {
    						-- выталкиваем элемент из стека в выходную строку
    				} 
    				И также удалим открывающуюся скобку из стека.
    	}
    	-- если символом оказалась бинарная операция R (приоритеты 2 и 3) {
    					Пока стек не пуст И приоритет операции со стека >= R {
    							-- выталкиваем верхний элемент стека в выходную строку
    					}
    					И также помещаем саму операцию R в стек.
    	}
    }
...
3. Теперь у нас есть возможность вычислить результат арифметического выражения по постфиксной записи.

    **На входе у нас есть постфиксная запись** нашего начального арифметического выражения. Цель - получить значение выражения. Будем также, как и в части перевода, пользоваться стеком для хранения чисел, которые подлежат вычислению с помощью следующего встреченного операнда.

    **Принцип работы** элементарен:

    ```c
    Создаем пустой стек

    Пока на входной постфиксной записи есть число или операнд {
    		-- читаем очередной символ R из постфиксной записи
    		
    		-- если R оказалось числом {
    					-- кладем в стек наше число R
    		}
    		-- если R оказалось знаком операции {
    					-- выталкиваем из стека верхнее число A
    					-- выталкиваем из стека верхнее число B
    					-- считаем значение C = A R B
    					-- кладем в стек получившийся результат С
    		}
    }

    Результат вычисления - единственный элемент в стеке.
    Берем его, удаляем его из стека и уничтожаем стек.
    ```

[:arrow_up: Back to contents](#Contents)
____

## Алгоритм Рабина и Карпа поиска подстроки в строке

Когда приходится работать со строками, возникает часто **задача поиска подстроки в строке** (каждый текст мы можем представить в виде одной строки, используя спец. символы \n и т.д.)

Пусть теперь задана строка *s* из *N* элементов и строка *q* из *M* элементов. Здесь мы подразумеваем, что *q* - не пуста и _M ≤ N_. Строку s мы назовем **текстом**, строку *q* - **образцом**.
**Ставится задача**: определить, содержит ли текст *s* образец *q*, и в случае положительного результата выдать позицию _k_ в тексте _s_, с которой начинается вхождение _q_ в _s_. **Задача прямого поиска** заключается в поиске индекса _k_, указывающего на первое с начала текста _s_ совпадение с шаблоном _q_.
...
P.S. такая сложность будет происходить, если поэлментое сравнение идет с начала шаблона и текст имеет вид
_АААААААААААААААААААААААААААААААА.....ААААААААВ_.

Но при этом шаблон содержит ту же идею, но с меньшим количеством "Ашек":
_ААААААА....ААААВ_

Одним из подходов к улучшению наивной реализации решения задачи поиска подстроки в строке является **алгоритм Рабина-Карпа**.

Вместо того, чтобы использовать более умный пропуск (Бойер-Мур, см. ниже), алгоритм Рабина—Карпа пытается **ускорить проверку эквивалентности образца с подстроками в тексте, используя хеш-функцию.** *Хеш-функция* — это функция, преобразующая каждую строку в числовое значение, называемое _хеш-значением_ (или просто хеш).
...
Однако, этого недостаточно для полной оптимизации задачи... Существуют две проблемы:

1. Между двумя различными строками **может произойти коллизия — совпадение их хешей.** Для избежания ложного срабатывания приходится всё равно проверять в итоге шаблон с подстрокой текста. Минимизация коллизий: подбор "хорошей" хеш-функции.
2. **Подсчет следующих хеш-значений**. Считать заново неэффективно, потеряем в скорости. 

**Для избежания частых коллизий используется полиномиальный хеш.** Для данного шаблона _q_ длины _M_ такой хеш будет определен следующим образом:

![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/84eeacbf-384a-4499-9adc-d769eeb3b729/Untitled.png](https://s3.us-west-2.amazonaws.com/secure.notion-static.com/84eeacbf-384a-4499-9adc-d769eeb3b729/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20200812%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20200812T132112Z&X-Amz-Expires=86400&X-Amz-Signature=44ae7079466be7b9af962081dedaf0bc80c72a3ba0d7d357a452b8c3c2d7bd0e&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22)

В данной формуле _x_ может быть любым числом от _1_ до _(p - 1)_. *p* берут большим числом для работы в кольце вычетов по модулю _p_ (Википедия). Для простоты можно использовать модифицированную хэш-функцию: см. лабу 1-1.

Соответственно, по этой формуле, единой для всей программы, **будут вычисляться и хеши подстрок в тексте.**

Разберем теперь **сам алгоритм Рабина-Карпа** поиска вхождения шаблона в текст.

1. Вычислим *patternHash* нашего шаблона _q_ длины _M_ перед началом поиска вхождения или его отсутствия, что алгоритм и в этом случае сможет показать.
2. Прикладываем левый край образца к левому краю текста. Пусть переменная *idx* будет отвечать за текущее положение "каретки" сравнения. В начале _idx = 0_. Вычисляем хеш-значение *currentHash* по формуле выше для подстроки текста длины _M_:
  ```c
  s[0 ... (M-1)]
  ```
3. Если хеши *currentHash* и *patternHash* совпали, то **необходимо! проверить отсутствие коллизии**, начиная с _idx_ позиции последовательным сравнением элементов образца _q[j]_ с символами потенциальной подстроки-совпадения _s[idx + j]_ слева направо.
4. Следующий шаг работы:
   - Если имеем ровно _М_ совпадений, то такой образец в тексте **найден** - алгоритм заканчивает работу.
   - Если такого совпадение не произошло, то нужно пойти дальше по тексту и пересчитать хеш для новой подстроки длины _M_ уже с индексами
  ```c
  s[(idx+1) ... (idx + M)];
  ```

   - Далее сделать инкремент idx и вернутся к шагу 3.

Само собой, **не забываем следить за границей текста** - если на очередном шаге перед пересчетом хеша оказалось, что *idx + M ≥  N*, то следующее сравнение уже произвести нельзя - **завершаем работу с ответом об отсутствии совпадения.**

**Оценка:**
Число сравнений зависит от сочетания хэш-функции, текста и образца (причем зависимость довольно серьезная!).
Что имеем:

- Изначальный подсчёт хешей выполняется за *O(M)*
- Каждая итерация выполняется за *O(1)* в среднем (здесь происходит пересчет хеш-значения за константное время). В цикле всего *N – M + 1* итераций.

Приходим к итоговому времени работы алгоритма в **среднем случае**: *O(N + M)*. В этом случае мы НЕ ПОДРАЗУМЕВАЕМ полную проверку на совпадение, т.е. под избежанием коллизии здесь понимается проверка совпадения, например, некоторых символов подстроки и шаблона на одинаковых позициях.
Однако если требуется исключить ложные срабатывания алгоритма полностью, т.е. нам придется проверить все полученные позиции вхождения, то в **худшем случае** итоговое время работы алгоритма будет *O(N * M)*.


[:arrow_up: Back to contents](#Contents)
____

## Алгоритм Бойера и Мура поиска подстроки в строке

P.S. Начало один в один совпадает с предыдущей статьей. Если вы с ним знакомы, смело переходите к самому алгоритму.
Когда приходится работать со строками, возникает часто **задача поиска подстроки в строке** (каждый текст мы можем представить в виде одной строки, используя спец. символы \n и т.д.)

Пусть теперь задана строка *s* из *N* элементов и строка *q* из *M* элементов. Здесь мы подразумеваем, что *q* - не пуста и _M ≤ N_. Строку s мы назовем **текстом**, строку *q* - **образцом**.
**Ставится задача**: определить, содержит ли текст *s* образец *q*, и в случае положительного результата выдать позицию _k_ в тексте _s_, с которой начинается вхождение _q_ в _s_. **Задача прямого поиска** заключается в поиске индекса _k_, указывающего на первое с начала текста _s_ совпадение с шаблоном _q_.
**Наивное решение** такой задачи: поэлементное сравнение всех символов *q[i]* из шаблона с символами *s[j]* текста. Такая реализация подразумевает сложность *O(N * M)* - слишком медленно, можно улучшить асимптотику.
Здесь мы приведем реализацию наивного алгоритма:
```c
int seekSubstring (char s[], char q[]) {
	int i, j, k, N, M;
	N = strlen(s);
	M = strlen(q);
	k = -1;
...
Для шаблона _аbсаbеаbсе_ (М = 10) имеем
| Символ | Таблица сдвигов | Значение в таблице | Примечание |
|:-----------------------:|:---------:|:---------:|:---------:|
| a | d['a'] | 3 | |
| b | d['b'] | 2 | |
| d | d['c'] | 1 | |
| e | d['e'] | 4 | предпоследнее вхождение |
| $ | d['$'] | 10 | $ - все остальные символы алфавита, не входящие в шаблон |

P.S. Далее везде понимается, что индексация начинается с 1. В конечной реализации алгоритма на языке Си она будет начинаться с 0.
Будем последовательно сравнивать шаблон _q_ с подстроками
```c
s[i – М + 1 ... i] /* в начале i = М */
```
Введем два рабочих индекса:
```c
 j = М, М – 1, ... , 1  /* пробегающий символы шаблона */
 k = i, ... , i – M + 1  /* пробегающий подстроку */
```
Оба индекса синхронно уменьшаются на каждом шаге. Что имеем далее?

- Если все символы _q_ совпадают с подстрокой (т. е. _j_ доходит до 0), то шаблон _q_ считается найденным в _s_ с позиции _k_ (k = i – M + 1)
...
```c
int seekSubstringBM(unsigned char s[], unsigned char q[]) { 
	int d[256];
	int i, j, k, N, M;
	N = strlen(s);
	M = strlen(q);
...
```
Что насчет сложности? Определение длин исходных строк выполняется в Си поиском заключительного нулевого символа и требует, таким образом, времени _N + M_. На этом шаге у нас определяется длина текста _N_ и шаблона _M_. Для построения таблицы сдвигов _d_ необходимо заполнить все значения таблицы сначала величиной, равной длине шаблона _M_, а потом занести значения в количестве _М_ в нужные позиции таблицы. Таким образом, таблица строится за время  _256 + М_.

Будем считать, что _М_ намного меньше _N_. Как правило, данный алгоритм требует значительно меньше _N_ сравнений. В благоприятных обстоятельствах, а именно если последний символ шаблона всегда попадает на несовпадающий символ текста, максимальное число сравнении символов есть _N / M_. В худшем случае сравнений будет _(N -M) * M_ (это случай, когда мы будем всегда сдвигаться на 1 и в самом конце найдем).
